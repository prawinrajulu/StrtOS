...
def run_sync_e2e():
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        client.post(f"{base_auth_url}/register", json={
            "organization_name": org_a, "full_name": "Admin Org A", "email": email_a, "password": password
        })
        login_a = client.post(f"{base_auth_url}/login", json={"email": email_a, "password": password})
        ...
        login_b = client.post(f"{base_auth_url}/login", json={"email": email_b, "password": password})
        token_b = login_b.json()["data"]["access_token"]
        headers_b = {"Authorization": f"Bearer {token_b}"}

        # 3. Org A creates a Client
        print(f"\n[3/9] Org A creating Client 'Helios SaaS'...")
        create_client_res = client.post(f"{base_client_url}", headers=headers_a, json={
            "name": "Helios SaaS",
            "industry": "Cloud SaaS",
        ...
        create_wf_res = client.post(f"{base_wf_url}", headers=headers_a, json={
        ...
        cross_get = client.get(f"{base_wf_url}/{wf_a_id}", headers=headers_b)
        ...
        cross_start = client.post(f"{base_wf_url}/{wf_a_id}/start", headers=headers_b)
        ...
        start_res = client.post(f"{base_wf_url}/{wf_a_id}/start", headers=headers_a)
        ...
        tasks_res = client.get(f"{base_wf_url}/{wf_a_id}/tasks", headers=headers_a)
        ...
        events_res = client.get(f"{base_wf_url}/{wf_a_id}/events", headers=headers_a)
        ...
        rep_res = client.get(f"http://localhost:8000/api/v1/ceo/report/{wf_a_id}", headers=headers_a)
